...
int main (int argc, char *argv[]){ 
  ...
  for(unsigned int i = 0; i < windows.size(); i++){
    x = (struct svm_node *) malloc(windows[i].size()*sizeof(struct svm_node));
    for(unsigned int j = 0; j < windows[i].size()-1; j++){
      x[j].value = (double) windows[i][j];
      x[j].index = (int) j+1;
    }
    x[windows[i].size()-1].index = -1;
    double pred = svm_predict(model,x);
    if(fabs(pred-windows[i][windows[i].size()-1]) < model->param.p) scores.push_back(0);
    else scores.push_back(fabs(pred-windows[i][windows[i].size()-1])-model->param.p);    
  }
